...
from models.unet_mv2d_blocks import (
...
class UNetMV2DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def __init__(
            ...
            "UpBlock2D", "CrossAttnUpBlockMV2D", "CrossAttnUpBlockMV2D", "CrossAttnUpBlockMV2D"),
        ...
        if moe_mlp:
            if num_task == 2:
                self.conv_out_normal = nn.Conv2d(
                    block_out_channels[0], out_channels, kernel_size=conv_out_kernel, padding=conv_out_padding
                )
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        ...
        CrossAttnDownBlock2D, CrossAttnDownBlockMV2D, DownBlock2D, CrossAttnUpBlock2D, CrossAttnUpBlockMV2D,
        UpBlock2D)):
    ...
    def forward(
        ...
        if not self.moe_mlp:
            ...
            if self.num_task == 3:
                sample_rgb, sample_normal, sample_depth = torch.chunk(sample, dim=0, chunks=3)
                sample_rgb = self.conv_in(sample_rgb)
                sample_normal = self.conv_in_normal(sample_normal)
                sample_depth = self.conv_in_depth(sample_depth)
                sample = torch.cat([sample_rgb, sample_normal, sample_depth], dim=0)
            elif self.num_task == 2:
                sample_rgb, sample_normal = torch.chunk(sample, dim=0, chunks=2)
                sample_rgb = self.conv_in(sample_rgb)
                sample_normal = self.conv_in_normal(sample_normal)
                sample = torch.cat([sample_rgb, sample_normal], dim=0)
        ...
        if not self.moe_mlp:
            ...
            if self.num_task ==3:
                ...
                sample_rgb = self.conv_out(sample_rgb)
                sample_normal = self.conv_out_normal(sample_normal)
                sample_depth = self.conv_out_depth(sample_depth)
                sample = torch.cat([sample_rgb, sample_normal, sample_depth], dim=0)
            elif self.num_task ==2:
                sample_rgb, sample_normal= torch.chunk(sample, dim=0, chunks=2)
                sample_rgb = self.conv_out(sample_rgb)
    ...
    def from_pretrained_2d(
            ...
            camera_embedding_type: str = 'e_de_da_sincos', num_views: int = 8, sample_size: int = 32,
            zero_init_conv_in: bool = True, zero_init_camera_projection: bool = False,
            projection_class_embeddings_input_dim: int = 12, cd_attention_last: bool = False,
            cd_attention_mid: bool = True, multiview_attention: bool = True,
            sparse_mv_attention: bool = False, mvcd_attention: bool = False,
            in_channels: int = 8, out_channels: int = 4, moe_mlp: bool = False, num_task: int = 3,
            **kwargs
    ):
        r"""
        ...
        if from_flax:
            ...
            if moe_mlp:
                for key in list(state_dict.keys()):  # Iterate over a copy of the keys
                    if '.ff' in key:
                        if num_task ==3:
                            new_key_normal = key.replace('.ff', '.ff_normal')
                            new_key_depth = key.replace('.ff', '.ff_depth')
                            state_dict[new_key_normal] = state_dict[key].clone()
                            state_dict[new_key_depth] = state_dict[key].clone()
                        elif num_task==2:
                            new_key_normal = key.replace('.ff', '.ff_normal')
                            state_dict[new_key_normal] = state_dict[key].clone()
            ...
            #conv_out_weight = state_dict['down_blocks.1.attentions.0.transformer_blocks.0.attn2.to_q.weight']


            #import numpy as np
            #np.savetxt('conv_out_weight00.txt', conv_out_weight.cpu().numpy().reshape(conv_out_weight.size(0), -1), fmt='%s')
            if 'conv_out.weight' in state_dict and moe_mlp:
                state_dict['conv_out_normal.weight'] = state_dict['conv_out.weight'].clone()
                state_dict['conv_out_depth.weight'] = state_dict['conv_out.weight'].clone()

                # Copy biases if they exist
                if 'conv_out.bias' in state_dict:
                    state_dict['conv_out_normal.bias'] = state_dict['conv_out.bias'].clone()
                    state_dict['conv_out_depth.bias'] = state_dict['conv_out.bias'].clone()


            if 'conv_in.weight' in state_dict and moe_mlp:
                state_dict['conv_in_normal.weight'] = state_dict['conv_in.weight'].clone()
                state_dict['conv_in_depth.weight'] = state_dict['conv_in.weight'].clone()

                # Copy biases if they exist
                if 'conv_in.bias' in state_dict:
                    state_dict['conv_in_normal.bias'] = state_dict['conv_in.bias'].clone()
                    state_dict['conv_in_depth.bias'] = state_dict['conv_in.bias'].clone()
            ...
            if any([key == 'conv_in_normal.weight' for key, _, _ in mismatched_keys]):
                # initialize from the original SD structure
                model.conv_in_normal.weight.data[:, :4] = conv_in_weight
            if any([key == 'conv_in_depth.weight' for key, _, _ in mismatched_keys]):
                # initialize from the original SD structure
                model.conv_in_depth.weight.data[:, :4] = conv_in_weight
            ...
            if zero_init_conv_in:
                ...
                if moe_mlp:
                    if num_task == 2:
                        model.conv_in_normal.weight.data[:, 4:] = 0.
                    elif num_task ==3:
                        model.conv_in_normal.weight.data[:, 4:] = 0.
                        model.conv_in_depth.weight.data[:, 4:] = 0.
