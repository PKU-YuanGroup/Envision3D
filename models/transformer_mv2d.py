...
class BasicMVTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        if not moe_mlp:
            ...
            self.ff = FeedForward(dim, dropout=dropout, activation_fn=activation_fn, final_dropout=final_dropout)
            if num_task ==3:
                self.ff_normal = FeedForward(dim, dropout=dropout, activation_fn=activation_fn, final_dropout=final_dropout)
                self.ff_depth = FeedForward(dim, dropout=dropout, activation_fn=activation_fn, final_dropout=final_dropout)
            elif num_task ==2:
                self.ff_normal = FeedForward(dim, dropout=dropout, activation_fn=activation_fn,
        ...
        if self.cd_attention_last:
            ...
            self.norm_joint_last = AdaLayerNorm(dim, num_embeds_ada_norm) if self.use_ada_layer_norm else nn.LayerNorm(
                dim)
        ...
        if self.cd_attention_mid:
            ...
            self.norm_joint_mid = AdaLayerNorm(dim, num_embeds_ada_norm) if self.use_ada_layer_norm else nn.LayerNorm(
                dim)
    ...
    def forward(
        ...
        if self.cd_attention_mid:
            ...
            hidden_states = self.attn_joint_mid(norm_hidden_states, num_task=self.num_task) + hidden_states
        ...
        if self._chunk_size is not None:
            ...
            if not self.moe_mlp:
                ...
                if self.num_task == 3:
                    norm_hidden_states, norm_hidden_states_normal, norm_hidden_states_depth = torch.chunk(
                        norm_hidden_states,
                        dim=0, chunks=3)
                    ff_output = self.ff(norm_hidden_states)
                    ff_output_normal = self.ff_normal(norm_hidden_states_normal)
                    ff_output_depth = self.ff_depth(norm_hidden_states_depth)
                    ff_output = torch.cat([ff_output, ff_output_normal, ff_output_depth], dim=0)
                elif self.num_task == 2:
                    norm_hidden_states, norm_hidden_states_normal = torch.chunk(norm_hidden_states, dim=0, chunks=2)
                    ff_output = self.ff(norm_hidden_states)
                    ff_output_normal = self.ff_normal(norm_hidden_states_normal)
                    ff_output = torch.cat([ff_output, ff_output_normal], dim=0)
        ...
        if self.cd_attention_last:
            norm_hidden_states = (
                self.norm_joint_last(hidden_states, timestep) if self.use_ada_layer_norm else self.norm_joint_last(
                    hidden_states)
            )
    ...
    def __call__(
        ...
        if multiview_attention:
            if not sparse_mv_attention:
                ...
                key_front = my_repeat(rearrange(key_raw, "(b t) d c -> b t d c", t=num_views)[:, 0, :, :],
                                      num_views)  # [(b t), d, c]
                value_front = my_repeat(rearrange(value_raw, "(b t) d c -> b t d c", t=num_views)[:, 0, :, :],
                                        num_views)
...
class XFormersJointAttnProcessor:
    ...
    def __call__(
        ...
        query = attn.to_q(hidden_states) #(3 b t) d c
        ...
        assert num_task == 3 or num_task == 2 # only support two tasks now
        if num_task ==3 :
            ...
            key = torch.cat([key_0, key_1], dim=1)  # (b t) 3d c
            value = torch.cat([value_0, value_1], dim=1)  # (b t) 3d c
            key = torch.cat([key] * 2, dim=0)  # (3 b t) 3d c
            value = torch.cat([value] * 2, dim=0)  # (3 b t) 3d c
