...
def load_a_prediction(root_dir,s2_root_dir, test_object, imSize, view_types, load_color=False, cam_pose_dir=None,
    ...
    RT_front = np.loadtxt(glob(os.path.join(cam_pose_dir, "0.txt"))[0])  # world2cam matrix
    RT_front_cv = RT_opengl2opencv(RT_front)  # convert normal from opengl to opencv
    step_size = 4
    views_list = [int(i * step_size) for i in range(8)]
    for idx in range(32):

        if idx in views_list:
            print(os.path.join(root_dir, test_object, f'rgb{idx}.png'))
            normal_filepath = os.path.join(root_dir, test_object, f'normal{idx}.png')
            # Load key frame
            if load_color:  # use bgr
                image = np.array(PIL.Image.open(normal_filepath.replace("normal", "rgb")).resize(imSize))[:, :, :3]
            ...
            color_mask = np.array(
                PIL.Image.open(os.path.join(root_dir, test_object, f'masked_rgb/rgb{idx}.png')).resize(
                    imSize))[:, :, 3]
            ...
            invalid_color_mask = color_mask < 250 * 0.5
            ...
            RT = np.loadtxt(os.path.join(cam_pose_dir, f'{idx}.txt'))  # world2cam matrix
            ...
            image_filepath = os.path.join(s2_root_dir, test_object, f'rgb{idx}.png')
            print(image_filepath)
            if load_color:  # use bgr
                image = np.array(PIL.Image.open(image_filepath).resize(imSize))[:, :, :3]
            color_mask = np.array(
                    PIL.Image.open(os.path.join(s2_root_dir, test_object, f's2_masked_rgb/rgb{idx}.png')).resize(
                        imSize))[:, :, 3]
            ...
            all_masks.append(mask) #dummy
            all_color_masks.append(color_mask)
            RT_cv = RT_opengl2opencv(RT)  # convert normal from opengl to opencv
            all_poses.append(inv_RT(RT_cv))  # cam2world
            all_w2cs.append(RT_cv)

            all_normals.append(normal_cam_cv) #dummy
            all_normals_world.append(normal_world)
...
class OrthoDatasetBase():
    def setup(self, config, split):
        ...
        self.s2_data_dir = self.config.s2_root_dir
        ...
        if self.config.cam_pose_dir is None:
            self.cam_pose_dir = "./datasets/camera_poses_w2c_gl"
        else:
        ...
        self.images_np, self.masks_np, self.normals_cam_np, self.normals_world_np, \
            self.pose_all_np, self.w2c_all_np, self.origins_np, self.directions_np,self.rgb_masks_np = load_a_prediction(
                self.data_dir, self.s2_data_dir, self.object_name, self.imSize, self.view_types,
